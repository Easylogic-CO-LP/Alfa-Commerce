...
if (!empty($saveOrder))
{
	$saveOrderingUrl = 'index.php?option=com_alfa&task=usergroups.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}

?>

<form action="<?php echo Route::_('index.php?option=com_alfa&view=usergroups'); ?>" method="post"
			...
			<?php echo LayoutHelper::render('joomla.searchtools.default', array('view' => $this)); ?>
				...
				<table class="table table-striped" id="usergroupList">
					...
					<th  scope="col" class="w-1 text-center">
						...
						<th class='left'>
							<?php echo HTMLHelper::_('searchtools.sort',  'COM_ALFA_USERGROUPS_NAME', 'a.name', $listDirn, $listOrder); ?>
						</th>
					...
					<?php foreach ($this->items as $i => $item) :
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group='1' data-transition>
							...
							<td class="text-center">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'usergroups.', $canChange, 'cb'); ?>
							</td>
							
							<td>
								<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'usergroups.', $canCheckin); ?>
								<?php endif; ?>
								<?php if ($canEdit) : ?>
