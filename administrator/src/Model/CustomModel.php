...
class CustomModel extends AdminModel
{
	 ...
	 * @since   1.0.1
	 */
	public function getTable($type = 'Custom', $prefix = 'Administrator', $config = array())
	{
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
				...
				if (!empty($table->categories))
				{
					if (is_array($table->categories))
					{
						$table->categories = implode(',', $table->categories);
					}
				}
				else
				{
				...
				if (!empty($table->items))
				{
					...
					{
						$table->items = implode(',', $table->items);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__alfa_customs');
				$max             = $db->loadResult();
