            ...
            foreach ($mediaData as $id => $media) {
                ...
                if ($isNew) {
                    ...
                    if(!empty($insertObject)){
                        $db->insertObject('#__alfa_items_media', $insertObject);
                    }
                }
                    ...
                    if ($toDelete) {
                        ...
                        if ($isFullDelete) {
                            ...
                            $query = $db->getQuery(true)
                                ...
                                ->from($db->quoteName('#__alfa_items_media'))
                        ...
                        $query = $db->getQuery(true)
                            ->delete($db->quoteName('#__alfa_items_media'))
                        ...
                        $db->updateObject('#__alfa_items_media', $obj, 'id');
                    }
                }
                $ordering++;
            }
...
//            $query = $db->getQuery(true)
//                ->select($db->quoteName('manufacturer_id'))
//                ->from($db->quoteName('#__alfa_items_manufacturers'))
//                ->whereIn($db->quoteName('item_id'), $itemIDs);
//            $db->setQuery($query);
//            $manufacturerID = $db->loadAssocList(); // get item manufacturers id

//            $query = $db->getQuery(true)
//                ->select($db->quoteName('path'))
//                ->from($db->quoteName('#__alfa_items_media'))
//                ->whereIn($db->quoteName('item_id'), $manufacturerID)
//                ->where($db->quoteName('origin') . ' = ' . $db->quote('manufacturer'));
//            $db->setQuery($query);
//            $manufacturerPath = $db->loadObjectList();
...
//            exit();
        ...
        private static function mediaImageGenerator(string $source_image_path, string $new_image_path, string $output_type, int $max_width, int $max_height, bool $resize_if_smaller, int $quality, array $allowedMimes, int $aspectratio = 0): false|string
        {
            ...
            if(!defined('NEW_IMAGE_MAX_WIDTH')) define('NEW_IMAGE_MAX_WIDTH', $max_width);
            if(!defined('NEW_IMAGE_MAX_HEIGHT')) define('NEW_IMAGE_MAX_HEIGHT', $max_height);
            ...
            switch ($output_type) {
                case 'jpg':
                case 'jpeg': $result = imagejpeg($image_to_output, $new_image_path, $quality); break;
                case 'png':
                    $pngQuality = (int) (9 - round(($quality / 100) * 9));
                    $result = imagepng($image_to_output, $new_image_path, $pngQuality);
                    break;
                case 'gif': $result = imagegif($image_to_output, $new_image_path); break;
                case 'webp': $result = imagewebp($image_to_output, $new_image_path, $quality); break;
                case 'avif': $result = imageavif($image_to_output, $new_image_path); break;
                default: $result = imagejpeg($image_to_output, $new_image_path, $quality);
            }

            // Cleanup
            imagedestroy($source_gd_image);
            if($resize_gd_image) imagedestroy($resize_gd_image);
