...
(function () {
    ...
    function initActions() {
        document.addEventListener('click', function (e) {
            ...
            // Resolve through the URL API so a tampered data-base can't smuggle a
            // javascript:/data: scheme into the iframe src (CodeQL: DOM-text-as-HTML).
            let url;
            try { url = new URL(base, window.location.origin); } catch (err) { return; }
            if (url.protocol !== 'http:' && url.protocol !== 'https:') return;

            if (tag) { url.searchParams.set('lang', tag); } else { url.searchParams.delete('lang'); }
