...
class com_alfaInstallerScript extends InstallerScript
{
	...
	private function installTemplates($parent)
	{
		...
		if (method_exists($parent, 'getManifest'))
		{
			$templates = $parent->getManifest()->templates;
		}
		else
		{
			$templates = $parent->get('manifest')->templates;
		}

		if (count($templates->children()))
		{
			foreach ($templates->children() as $template)
			{
				$templateName = (string) $template['template'];
				$path = $installation_folder . '/templates/' . $templateName;
				$installer = new Installer;

				if (!$this->isAlreadyInstalled('template', $templateName))
				{
					$result = $installer->install($path);
				}
				else
				...
				if ($result)
				{
					$app->enqueueMessage('Template ' . $templateName . ' was installed successfully');
				}
				else
				{
					$app->enqueueMessage('There was an issue installing the template ' . $templateName, 'error');
				}
			}
		}
	}
		...
		if (count($templates->children()))
		{
			...
			foreach ($templates->children() as $template)
			{
				...
				$query
					...
					->where(
						array(
							'type LIKE ' . $db->quote('template'),
							'element LIKE ' . $db->quote($templateName)
						)
					);
				$db->setQuery($query);
				$extension = $db->loadResult();
				...
				{
					$installer = new Installer;
					$result = $installer->uninstall('template', $extension);

					if ($result)
					{
						$app->enqueueMessage('Template ' . $templateName . ' was uninstalled successfully');
					}
					else
					{
						$app->enqueueMessage('There was an issue uninstalling the template ' . $templateName, 'error');
					}
				}
			}
		}
