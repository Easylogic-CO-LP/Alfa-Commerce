...
final class Klarna extends PaymentsPlugin
{
    ...
    public function onOrderProcessView($event): void
    {
        ...
        $result = $input->getString('klarna_result', '');
        ...
        if ($result === 'error') {
            ...
            $event->setLayoutData([
                'error' => $input->getString('klarna_result_msg', ''),
    ...
    public function onPaymentResponse($event): void
    {
        ...
        if (empty($sid)) {
            $event->setRedirectUrl(
                Route::_($this->getProcessPageUrl() . '&klarna_result=cancelled', false, Route::TLS_FORCE, true),
            );
            return;
        }
    ...
    private function redirectToKlarna($event, object $order): void
    {
        try {
            ...
            $hpp = $klarna->createHppSession(
                ...
                urls: [
                    ...
                    'notification' => Route::_(
                        'index.php?option=com_alfa&task=plugin.trigger&type=alfa-payments&name=klarna&func=notify'
                        . '&order_id=' . $order->id
                        . '&sid={session.id}',
                        false,
                        Route::TLS_FORCE,
            ...
            $event->setRedirectUrl(
                Route::_($this->getProcessPageUrl() . '&klarna_result=error', false, Route::TLS_FORCE, true),
            );
        } catch (Exception $e) {
