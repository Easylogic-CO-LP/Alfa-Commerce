<?php
	...
	final class Viva extends PaymentsPlugin
	{
		...
		private function isTestMode(object $order): bool      { return $this->methodParams($order)->get('mode', 'test') === 'test'; }
		private function clientId(object $order): string      { return (string) $this->methodParams($order)->get('client_id', ''); }
		private function clientSecret(object $order): string  { return (string) $this->methodParams($order)->get('client_secret', ''); }
		private function sourceCode(object $order): string    { return (string) $this->methodParams($order)->get('source_code', ''); }
		private function isPreauth(object $order): bool       { return $this->methodParams($order)->get('intent', 'charge') === 'authorize'; }
		private function maxInstallments(object $order): int  { return (int) $this->methodParams($order)->get('max_installments', 0); }

		// Classic API credentials — for refund/void via Basic Auth (no portal permission needed)
		private function merchantId(object $order): string    { return (string) $this->methodParams($order)->get('merchant_id', ''); }
		private function apiKey(object $order): string        { return (string) $this->methodParams($order)->get('api_key', ''); }
		...
		private function handleViewLogs($event): void
		{
			...
			$query = $db->getQuery(true)
				->select('*')
				->from($db->quoteName('#__alfa_payments_viva_logs'))
				->where($db->quoteName('id_order') . ' = ' . (int) $order->id)
