    ...
    data.forEach((currentData, index) => {
        ...
        cancelDeliveryButtons.forEach((button, index) => {
            ...
            if (cancelDeliveryButtons[index].getAttribute('data-parcel-id') === currentData.id) {
                ...
                disableCancelButton(cancelDeliveryButtons[index], false, currentData.id); //enable cancel buttons
            }
        })
    })
}
...
function disableCancelButton(button, disable, parcelID = 0) {
    if (disable) {
        ...
        if(!button.hasEventListener) {  // Prevent the addition of multiple event listeners.
            console.log("Didn't have event listener, adding..");
            button.addEventListener("click", () => cancelIndividualParcel(parcelID));
            button.hasEventListener = true;
        }
        else{
            console.log("There was an event listener, did not add another.");
        }
    }
}